...
fn main() -> Result<(), Box<dyn std::error::Error>> { //pour renvoyer n'importe quelle erreur
    let args = Args::parse();
    ...
    if args.read {
        ...
        lireFile(&file_path, offset, size)?;
    }

    // write
    if let Some(hex_string) = args.write {
        ecrireFile(&file_path, &hex_string, offset)?;
    }

    Ok(())
}

pub fn lireFile(file_path: &str,offset: u64, size: usize,) -> Result<(), Box<dyn std::error::Error>> {
...
pub fn ecrireFile(file_path: &str,hex_string: &str,offset: u64,) -> Result<(), Box<dyn std::error::Error>> {
    ...
    print!("Hex:  ");
    for b in &bytes {  //ecrit les bytes par deux
        print!("{:02x} ", b);
...
fn hex_to_bytes(hex: &str) -> Result<Vec<u8>, Box<dyn std::error::Error>> {
    ...
    if clean.len() % 2 != 0 { //la chaine dois etre paire
